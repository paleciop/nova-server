...
var QueryStringPattern = (function() {
  ...
  QueryStringPattern.prototype.matches = function(queryString) {
    ...
    var requiredParam,
      requiredParams = [].concat(this.params);
    while ((requiredParam = requiredParams.shift())) {
      if (!givenParams.hasOwnProperty(requiredParam.key)) return false;
      if (
        requiredParam.value &&
        givenParams[requiredParam.key] != requiredParam.value
      )
        return false;
    }
    if (!this.allowWildcards && this.params.length) {
      if (Object.getOwnPropertyNames(givenParams).length > this.params.length)
        return false;
    }
    return true;
  };
  ...
  QueryStringPattern.prototype.match = function(queryString) {
    ...
    Object.keys(parsedQueryString).forEach(function(key) {
      ...
      if (namedParams[key]) {
        data.namedQueryParams[namedParams[key]] = data.namedParams[
          namedParams[key]
        ] = value;
      }
    });
...
var RoutePattern = (function() {
  ...
  RoutePattern.prototype.matches = function(location) {
    ...
    return (
      (!this.pathPattern || this.pathPattern.matches(loc.path)) &&
      (!this.queryStringPattern ||
        this.queryStringPattern.matches(loc.queryString)) &&
  ...
  RoutePattern.fromString = function(routeString) {
    ...
    return new RoutePattern({
      ...
      queryStringPattern:
        matchQueryString && QueryStringPattern.fromString(parts.queryString),
